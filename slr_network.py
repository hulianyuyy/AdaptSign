...
class SLRModel(nn.Module):
    def __init__(
        ...
        self.conv1d = TemporalConv(input_size=512, #768 for ViT-L/14, 512 for ViT-B/16, 512 for ViT-B/32, 1024 for RN50
    ...
    def forward(self, x, len_x, label=None, label_lgt=None):
        if len(x.shape) == 5:
            ...
            #with torch.no_grad():
            framewise = self.conv2d.encode_image(inputs, temp) #self.masked_bn(inputs, len_x)
                #if not self.training:
                #    framewise = framewise.float()
