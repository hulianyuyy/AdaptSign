...
class Processor():
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            dev_wer = seq_eval(self.arg, self.data_loader["dev"], self.model, self.device, "dev", 6667, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
            test_wer = seq_eval(self.arg, self.data_loader["test"], self.model, self.device, "test", 6667, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
    ...
    def adjust_lr(self, model):
        ...
        for name, m in model.named_modules():
            if 'resblocks_no_grad' in name:  #for clip
                if len(list(m.parameters()))>0:
                    if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv3d) or isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, torch.nn.Conv1d) or isinstance(m, torch.nn.BatchNorm1d) or isinstance(m, torch.nn.BatchNorm3d) or isinstance(m, torch.nn.Linear) or isinstance(m, nn.LayerNorm) or isinstance(m, nn.MultiheadAttention) :
                        if len(list(m.parameters()))>0:
                ...
                #if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv3d) or isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, torch.nn.Conv1d) or isinstance(m, torch.nn.BatchNorm1d) or isinstance(m, torch.nn.BatchNorm3d) or isinstance(m, torch.nn.Linear) :
                ps = list(m.parameters())
                if len(ps) == 1:  # Conv, FC
                    normal_weight.append(ps[0])
                elif len(ps) == 2:  #BN, Conv, FC
                    normal_weight.append(ps[0])
                    normal_bias.append(ps[1])
                else:
                    print(f'assign lr for unusual network components : {type(m)}')
                    normal_weight.extend(ps)  #For LSTM with len(params)=4*N
            elif len(list(m.parameters()))>0:
                pass #print(type(m))  # mid layers
